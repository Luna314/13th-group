...
class MaoYan(Spider):
    ...
    def start_requests(self):
        for eachsite in ['http://piaofang.maoyan.com/rankings/year?year=201{}&limit=100&tab=8'.format(i) for i in range(5, 9)]:
            yield Request(eachsite, dont_filter=True, encoding='utf-8', callback=self.get_url)

#获取电影ID 进入电影详情页
    def get_url(self, response):
        body = response.text
        movielist = re.findall('<ul class="row" data-com="hrefTo,href:\'/movie/(\d+)\'">', body)
        for i in movielist:
