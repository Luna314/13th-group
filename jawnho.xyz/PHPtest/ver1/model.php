<?php
    ...
    function model_4($con, $Params, $sqlGen) {
        ...
        $keyParams ->setKeys(
            ...
            $Where = array([["gender", "'".$Params["sex"]."'"], 
                ["release_date", dateGen(true, $Params["year"]), 
                    "LIKE"]]
            ),
            $Dist = true,
            $func = "", 
        ...
        $ret2 = array();
        foreach ($con->query($sql) as $row) {
            array_push($ret1, $row["performer"]);
            array_push($ret2, $row["count(*)"]);
        }
        
        return array("performer"=>$ret1, "num"=>$ret2);
    }
