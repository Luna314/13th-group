...
class MovieBasicItem(scrapy.Item):
    ...
    #all_performers = scrapy.Field()

    def save(self, cursor):
        sql = "INSERT IGNORE INTO movie_basic (movie_name, movie_type, director, release_date, total_box) VALUES (%s, %s, %s, %s, %s)"
        t1 = (self['movie_name'], self['movie_type'], self['director'], self['release_date'], self['total_box'])
        cursor.execute(sql, t1)
        update_sql = "UPDATE movie_basic SET total_box=%s WHERE movie_name=%s"
        t2 = (self['total_box'], self['movie_name'])
